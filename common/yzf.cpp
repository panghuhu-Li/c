...
typedef struct FCB {
    ...
    char exname[3];    //文件扩展名
    unsigned char
        attribute;  //文件属性字段：为简单起见，我们只为文件设置了两种属性：
...
typedef struct USEROPEN {
    ...
    char exname[3];    //文件扩展名
    unsigned char
        attribute;  //文件属性：值为0时表示目录文件，值为1时表示数据文件
    unsigned short time;   //文件创建时间
    unsigned short date;   //文件创建日期
    unsigned short first;  //文件起始盘块号
    unsigned long
        length;  //文件长度（对数据文件是字节数，对目录文件可以是目录项个数）
    char free;  //表示目录项是否为空，若值为0，表示空，值为1，表示已分配
                //前面内容是文件的FCB中的内容。
    ...
    char dir[80];  //相应打开文件所在的目录名，这样方便快速检查出
                   //指定文件是否已经打开
...
char myfilename[] = "myfilsys";  //文件系统的文件名
 ...
 * @brief 创建子目录
 *
 * @param dirname 创建子目录路径
 */
void my_mkdir(char* dirname);
/**
 * @brief 删除子目录
 *
 * @param dirname 删除的子目录路径
 */
 ...
 * @brief 打开文件
 *
 * @param filename 打开文件的路径
 * @return int 成功返回对应文件描述符，失败返回-1
 */
int my_open(char* filename);
/**
 * @brief 关闭文件
 *
 * @param fd 要关闭文件的对应描述符
 * @return int 成功返回对应父目录对应描述符，失败返回-1
 */
int my_close(int fd);
/**
 * @brief 写文件
 *
 * @param fd 要写入文件对应文件描述符
 * @return int 成功返回对应实际写入文件的字节数，文件不存在或者错误返回-1
 */
int my_write(int fd);
/**
 * @brief 实际写文件
 ...
 * @param text 写入内容
 * @param len 写入内容的长度
 * @param wstyle 文件写入模式，1.cut write 2.cover write 3.add write
 * @return int 错误返回-1，否则返回实际写入字节数
 */
int do_write(int fd, char* text, int len, char wstyle);
/**
 * @brief 读文件
 *
 * @param fd 要读文件对应的文件描述符
 * @param len 读取的文件长度
 * @return int 文件不存在返回-1，成功返回实际读取字节数
 */
 ...
 *
 * @param fd 读取的文件描述符
 * @param len text容器对应的大小
 * @param text 读取的内容存放的容器
 * @return int 实际读取的长度
 */
 ...
 * @brief 寻找空闲盘块
 *
 * @return unsigned short 成功：返回对应找到的文件盘块，错误：返回-1
 */
...
void my_format() {
    ...
    strcpy(blk0->information,
           "My FileSystem Ver  \n Blocksize=1KB Wholesize=1000KB Blocknum=1000 "
           "RootBlocknum=2\n");
    blk0->root = 5;
    ...
    root->attribute = 0x28;
    root->time =
        nowtime->tm_hour * 2048 + nowtime->tm_min * 32 + nowtime->tm_sec / 2;
    root->date = (nowtime->tm_year - 80) * 512 + (nowtime->tm_mon + 1) * 32 +
                 nowtime->tm_mday;
    root->first = 5;
...
void my_cd(char* dirname) {
    ...
    dir = strtok(
        dirname,
        "\\");  //分解字符串为一组字符串。dirname为要分解的字符串，"\\"为分隔符字符串
    if (strcmp(dir, ".") == 0)
        return;
...
void my_mkdir(char* dirname) {
    ...
    fcbptr->time =
        nowtime->tm_hour * 2048 + nowtime->tm_min * 32 + nowtime->tm_sec / 2;
    fcbptr->date = (nowtime->tm_year - 80) * 512 + (nowtime->tm_mon + 1) * 32 +
                   nowtime->tm_mday;
    fcbptr->first = blkno;
    fcbptr->length = 2 * sizeof(fcb);
    fcbptr->free = 1;
    openfilelist[curdir].count =
        i * sizeof(fcb);  //把当前目录的文件读写指针定位到文件末尾
    do_write(curdir, (char*)fcbptr, sizeof(fcb),
             2);  //从指针 fcbptr开始写一个fcb大小的内容到当前目录文件末尾
    ...
    openfilelist[curdir].count = 0;
    do_write(curdir, (char*)fcbptr, sizeof(fcb),
             2);  //更新当前目录文件的内容openfilelist[curdir].fcbstate = 1;
}
void my_rmdir(char* dirname) {
    fcb *fcbptr, *fcbptr2;
    ...
    rbn2 =
        do_read(fd, openfilelist[fd].length, text2);  //读取要删除的目录的内容
    fcbptr2 = (fcb*)text2;
    for (j = 0; j < rbn2 / sizeof(fcb); j++)  //判断要删除目录是否为空
    {
        if (strcmp(fcbptr2->filename, ".") && strcmp(fcbptr2->filename, "..") &&
            ...
            my_close(fd);
            printf("Error,the directory is not empty.In");
            return;
        }
        fcbptr2++;
    }
    ...
    my_close(fd);
    strcpy(fcbptr->filename, "");  //修改已删除目录在当前目录的fcb 的属性
    fcbptr->free = 0;
        ...
        if (fcbptr->free) {
            if (fcbptr->attribute & 0x20) {
                ...
                printf("%s.%s\t\t%dB\t\t%d%d%d\t%02d:%02d:%02d\t\n",
                       ...
                       fcbptr->date & 0x01f, fcbptr->time >> 11,
...
void my_create(char* filename) {
    fcb* fcbptr;
    fat *fat1, *fat2;
    char *fname, *exname, textp[MAXTEXT];
    char* text;
    unsigned short blkno;
    int rbn, i;
    time_t now;
...
void my_rm(char* filename) {
    ...
    if (!exname) {
        printf("Error, removing file must have an extern name.\n");
        return;
    }
    openfilelist[curdir].count = 0;
    rbn = do_read(curdir, openfilelist[curdir].length, text);
...
int my_open(char* filename) {
    ...
    for (i = 0; i < MAXOPENFILE; i++) {
        if (strcmp(openfilelist[i].filename, fname) == 0 &&
            strcmp(openfilelist[i].exname, exname) == 0 && i != curdir) {
    ...
    for (i = 0; i < rbn / sizeof(fcb); i++) {
        if (strcmp(fcbptr->filename, fname) == 0 &&
            strcmp(fcbptr->exname, exname) == 0 && i != curdir) {
            break;
        }
        fcbptr++;
    }
...
int my_write(int fd) {
    ...
    while (1) {
        printf(
            "Please enter the number of write style:\n1.cut write\t2.cover "
            "write\t3.addwrite\n");
        scanf("%d", &wstyle);
...
int do_write(int fd, char* text, int len, char wstyle) {
    ...
    while (blkoff >= BLOCKSIZE) {
        ...
        blkoff =
            blkoff - BLOCKSIZE;  //让 blkoff定位到文件最后一个磁盘块的读写位置
    }
    ll = 0;           //实际写的字节数
    while (ll < len)  // len 是用户输入的字节数
    {
...
int do_read(int fd, int len, char* text) {
    ...
    fatptr = fat1 + blkno;
    while (blkoff >= BLOCKSIZE)  // blkoff 为最后一块盘块剩余的容量
    {
        blkno = fatptr->id;
...
int main() {
    char cmd[15][10] = {"cd",   "mkdir", "rmdir", "Is",   "create", "rm",
                        "open", "close", "write", "read", "exit"};
    char s[30], *sp;
    char* string = s;
    int cmdn, flag = 1, i;
    startsys();
    printf("***********File    System   MJH1.0********\n\n");
    printf("命令名\t\t命令参数\t\t命令说明\n\n");
    printf("cd\t\t目录名(路径名)\t\t切换当前目录到指定目录\n");
    printf("mkdir\t\t目录名\t\t\t在当前目录创建新目录\n");
    printf("rmdir\t\t目录名\t\t\t在当前目录删除指定目录\n");
    printf("Is\t\t无\t\t\t显示当前目录下的目录和文件\n");
            ...
            switch (cmdn) {
                case 0:
                    ...
                    if (sp && (openfilelist[curdir].attribute & 0x20))
                        ...
                        printf("1Please input the right command.\n");
                    break;
                case 1:
                    sp = strtok(NULL, " ");
                    if (sp && (openfilelist[curdir].attribute & 0x20))
                        ...
                        printf("2Please input the right command.\n");
                    break;
                case 2:
                    sp = strtok(NULL, "");
                    if (sp && (openfilelist[curdir].attribute & 0x20))
                        ...
                        printf("3Please input the right command.\n");
                    break;
                case 3:
                    if (openfilelist[curdir].attribute & 0x20)
                        my_ls();
                    else
                        printf("4Please input the right command.\n");
                    break;
                case 4:
                    sp = strtok(NULL, " ");
                    if (sp && (openfilelist[curdir].attribute & 0x20))
                        ...
                        printf("5Please input the right command.\n");
                    break;
                case 5:
                    sp = strtok(NULL, " ");
                    if (sp && (openfilelist[curdir].attribute & 0x20))
                        ...
                        printf("6Please input the right command.\n");
                    break;
                case 6:
                    sp = strtok(NULL, " ");
                    if (sp && (openfilelist[curdir].attribute & 0x20)) {
                        ...
                        printf("7Please input the right command.\n");
                    break;
                case 7:
                    if (!(openfilelist[curdir].attribute & 0x20))
                        curdir = my_close(curdir);
                ...
                case 10:
                    if (openfilelist[curdir].attribute & 0x20) {
                        ...
                        printf("8Please input the right command.\n");
                    break;
                default:
                    printf("9Please input the right command.\n");
                    break;
            }
        }
    }
